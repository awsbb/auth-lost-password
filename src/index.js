...
const sendLostPasswordEmail = ({ email, token }) => {
  return new Promise((resolve, reject) => {
    const subject = `Password Lost For [${process.env.EXTERNAL_NAME}]`;
    const lostPasswordLink = `${process.env.RESET_PAGE}?email=${encodeURIComponent(email)}&lost=${token}`;
    const HTML = `
    <html>
    <head>
    ...
    <body>
      Please <a href="${lostPasswordLink}">click here to reset your password</a> or copy & paste the following link in a browser:<br><br><a href="${lostPasswordLink}">${lostPasswordLink}</a>
    </body>
    </html>
    `;
    SES.sendEmail({
